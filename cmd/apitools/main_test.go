...
func TestCatalogStatsOutputAndJSON(t *testing.T) {
	...
	for _, expected := range []string{"Provider protocols: 266 provider(s)", "OpenAPI", "81", "Swagger", "10", "Smithy", "15", "Google Discovery", "18", "Human docs", "140", "Artifact registry", "openapi", "Refresh artifacts", "valid-swagger"} {
		if !strings.Contains(out.String(), expected) {
	...
	for _, expected := range []string{`"provider_count": 266`, `"protocol": "openapi"`, `"count": 81`, `"protocol": "swagger"`, `"count": 10`, `"protocol": "smithy"`, `"count": 15`, `"protocol": "google-discovery"`, `"count": 18`, `"artifact_registry"`, `"kind": "openapi"`, `"status": "valid-swagger"`} {
		if !strings.Contains(out.String(), expected) {
