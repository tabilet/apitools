...
func TestCatalogStatsOutputAndJSON(t *testing.T) {
	...
	for _, expected := range []string{"Provider protocols: 129 provider(s)", "OpenAPI", "47", "Swagger", "4", "Human docs", "69", "Artifact registry", "openapi", "Refresh artifacts", "valid-swagger"} {
		if !strings.Contains(out.String(), expected) {
	...
	for _, expected := range []string{`"provider_count": 129`, `"protocol": "openapi"`, `"count": 47`, `"artifact_registry"`, `"kind": "openapi"`, `"status": "valid-swagger"`} {
		if !strings.Contains(out.String(), expected) {
