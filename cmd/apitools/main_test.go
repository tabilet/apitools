...
func TestCatalogStatsOutputAndJSON(t *testing.T) {
	...
	for _, expected := range []string{"Provider protocols: 91 provider(s)", "OpenAPI", "38", "Swagger", "4", "Human docs", "40", "Artifact registry", "openapi", "Refresh artifacts", "valid-swagger"} {
		if !strings.Contains(out.String(), expected) {
	...
	for _, expected := range []string{`"provider_count": 91`, `"protocol": "openapi"`, `"count": 38`, `"artifact_registry"`, `"kind": "openapi"`, `"status": "valid-swagger"`} {
		if !strings.Contains(out.String(), expected) {
