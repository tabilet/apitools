...
func main() {
	doc := map[string]any{
		...
		"x-apitools-overlay": map[string]any{
			...
			"source_note": "Google Calendar publishes an official Discovery document, but no official OpenAPI document is recorded in the apitools catalog. The reviewed overlay includes calendar, event, free/busy, and event notification channel operations.",
		},
		"components": map[string]any{
			"securitySchemes": map[string]any{
				"googleOAuth2": googleOAuthScheme(map[string]string{
					...
					"https://www.googleapis.com/auth/calendar.events.freebusy":       "Read free/busy information for calendar events.",
					...
					"https://www.googleapis.com/auth/calendar.freebusy":              "Read free/busy information.",
				}),
			},
			"schemas": map[string]any{
				...
				"ChannelRequest":   objectSchema(),
				"ChannelStop":      objectSchema(),
				"EmptyResponse":    objectSchema(),
				"Event":            objectSchema(),
				"EventList":        objectSchema(),
				"EventRequest":     objectSchema(),
				"FreeBusyRequest":  objectSchema(),
				"FreeBusyResponse": objectSchema(),
				"Error":            objectSchema(),
			},
		},
		...
		"paths": map[string]any{
			...
			"/calendars/{calendarId}/events": map[string]any{
				...
				}, "", "#/components/schemas/EventList", []string{"https://www.googleapis.com/auth/calendar.events.readonly"}),
			...
			"/calendars/{calendarId}/events/watch": map[string]any{
				"post": operation("watchGoogleCalendarEvents", "Watch Google Calendar event changes", []map[string]any{
					pathParam("calendarId", "Calendar identifier. Use primary for the signed-in user's primary calendar."),
					queryParam("eventTypes", "Event type filter; this parameter can be repeated by callers that need multiple event types."),
				}, "#/components/schemas/ChannelRequest", "#/components/schemas/Channel", []string{"https://www.googleapis.com/auth/calendar.events.readonly"}),
			},
			"/channels/stop": map[string]any{
				"post": operation("stopGoogleCalendarChannel", "Stop a Google Calendar notification channel", nil, "#/components/schemas/ChannelStop", "#/components/schemas/EmptyResponse", []string{"https://www.googleapis.com/auth/calendar.events.readonly"}),
			},
			"/freeBusy": map[string]any{
				"post": operation("queryGoogleCalendarFreeBusy", "Query Google Calendar free/busy information", nil, "#/components/schemas/FreeBusyRequest", "#/components/schemas/FreeBusyResponse", []string{"https://www.googleapis.com/auth/calendar.freebusy"}),
			},
		},
	}
