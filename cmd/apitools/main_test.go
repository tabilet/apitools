...
func TestCatalogStatsOutputAndJSON(t *testing.T) {
	...
	for _, expected := range []string{"Provider protocols: 66 provider(s)", "OpenAPI", "29", "Swagger", "4", "Human docs", "24", "Artifact registry", "openapi", "Refresh artifacts", "valid-swagger"} {
		if !strings.Contains(out.String(), expected) {
	...
	for _, expected := range []string{`"provider_count": 66`, `"protocol": "openapi"`, `"count": 29`, `"artifact_registry"`, `"kind": "openapi"`, `"status": "valid-swagger"`} {
		if !strings.Contains(out.String(), expected) {
